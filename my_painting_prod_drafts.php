...
if (($_GET['action'] ?? '') === 'delete' && isset($_GET['id'])) {
    $stmt = $pdo->prepare("DELETE FROM t_painting_prod_header WHERE id = ? AND status = 'draft'");
    $stmt->execute([(int)$_GET['id']]);
    header('Location: my_painting_prod_drafts.php?deleted=1');
    exit;
}

$sql = "SELECT h.*, d.name AS department_name,
               (SELECT COUNT(*) FROM t_painting_prod_line l WHERE l.header_id = h.id) AS line_count
...
$page_subtitle = 'Painting daily reports not yet submitted';
...
<div class="cs-card-list">
    ...
    <div class="cs-card">
        ...
        <div class="cs-card-body">
            <div class="cs-card-title"><?= htmlspecialchars($row['department_name']) ?> &middot; Painting Daily Report</div>
        ...
        <div class="cs-card-actions">
            <a href="painting_prod_list.php?department_id=<?= $row['department_id'] ?>&draft_id=<?= $row['id'] ?>" class="cs-view-btn">Continue</a>
            <a href="my_painting_prod_drafts.php?action=delete&id=<?= $row['id'] ?>" onclick="return confirm('Delete this draft?')" class="cs-delete-link">Delete</a>
        </div>
    </div>
    <?php endforeach; ?>
