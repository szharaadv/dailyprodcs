...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save') {
    ...
    if ($checking_item === '' || !$model_id) {
        ...
        if ($id !== '') {
            $stmt = $pdo->prepare('UPDATE m_assy_checklist_item SET model_id=?, checking_item=?, standard=?, standard_min=?, standard_max=?, sort_order=? WHERE id=?');
            $stmt->execute([$model_id, $checking_item, import_nz($standard), import_nz($standard_min), import_nz($standard_max), $sort_order, (int)$id]);
        } else {
...
if (($_GET['action'] ?? '') === 'delete' && isset($_GET['id'])) {
    try {
        ...
        $stmt->execute([(int)$_GET['id']]);
        header('Location: assy_checklist_items.php?model_id=' . (int)($_GET['model_id'] ?? 0) . '&deleted=1');
        exit;
...
$modelStmt = $pdo->prepare('SELECT * FROM m_assy_model WHERE department_id = ? ORDER BY sort_order, id');
...
$base_url = '../';
$active_nav = 'config-assy-checklist-item';
$section_route = 'assembly_list.php';
$page_title = 'Checking Item';
$page_subtitle = 'Master Data · Assembling Checking Items';
...
<div class="list-toolbar">
    ...
    <?php if ($selectedModel): ?>
        <a class="btn" href="assy_checklist_items.php?model_id=<?= $selected_model_id ?>&action=new">+ Add Checking Item</a>
    <?php endif; ?>
</div>
    ...
    <div class="modal-card">
        ...
        <form method="post">
            ...
            <div class="modal-actions">
                <a href="assy_checklist_items.php?model_id=<?= $selected_model_id ?>" class="btn btn-secondary">Cancel</a>
