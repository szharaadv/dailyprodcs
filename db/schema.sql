...
CREATE TABLE `m_user` (
  ...
  `title` varchar(50) NULL DEFAULT NULL COMMENT 'Job role used by the FO Pump Check sign-off flow: Operator/Staff = Checker, Foreman, Supervisor.',
  `email` varchar(150) NULL DEFAULT NULL COMMENT 'Optional — used only to notify this person when a checksheet reaches their sign-off level. NULL = in-app only.',
  `is_active` tinyint(1) NOT NULL DEFAULT 1,
...
CREATE TABLE `m_checksheet_section` (
  ...
  -- Document metadata for the Excel export (editable in admin/section_docs.php).
  -- doc_title = exact report heading; empty → "<NAME> MONTHLY CHECK SHEET REPORT".
  `doc_title` varchar(150) NULL DEFAULT NULL,
  `doc_no` varchar(60) NULL DEFAULT NULL,
  `doc_rev` varchar(20) NULL DEFAULT NULL,
  `doc_date` varchar(20) NULL DEFAULT NULL,
  `sort_order` int(11) NOT NULL DEFAULT 0,
  `is_active` tinyint(1) NOT NULL DEFAULT 1,
  PRIMARY KEY (`id`),
...
CREATE TABLE `t_checksheet_header` (
  ...
  -- Role sign-off trail (checker stamped on submit; foreman/supervisor sign
  ...
  KEY `fk_checksheetheader_foreman` (`foreman_id`),
  KEY `fk_checksheetheader_supervisor` (`supervisor_id`),
...
CREATE TABLE `t_assy_header` (
  ...
  `checker_id` int(11) NOT NULL,
  -- Role sign-off trail (checker line is stamped on submit; foreman/supervisor
  ...
  KEY `fk_assyheader_foreman` (`foreman_id`),
  KEY `fk_assyheader_supervisor` (`supervisor_id`),
...
CREATE TABLE `t_fopump_check_header` (
  ...
  `department_id` int(11) NOT NULL,
  `model_id` int(11) NOT NULL,
  -- One record per model PER DAY: the sheet resets each day, past days kept as
  -- history. Fill/sign-off always target today's record.
  `tanggal` date NOT NULL,
  `prod_date_code` varchar(50) NULL DEFAULT NULL,
  -- Role-based sign-off trail: each of checker/foreman/supervisor is stamped
  -- with the user id + time when that person signs. checker_id is nullable
  -- because a foreman/supervisor may sign before the operator has.
  `checker_id` int(11) NULL DEFAULT NULL,
  `checker_at` datetime NULL DEFAULT NULL,
  `foreman_id` int(11) NULL DEFAULT NULL,
  `foreman_at` datetime NULL DEFAULT NULL,
