...
if (($_GET['action'] ?? '') === 'delete' && isset($_GET['id'])) {
    $stmt = $pdo->prepare("DELETE FROM t_fopump_check_header WHERE id = ? AND status = 'draft'");
    $stmt->execute([(int)$_GET['id']]);
    header('Location: my_fopump_check_drafts.php?deleted=1');
    exit;
}
...
$page_subtitle = 'FO Pump check sheets not yet submitted';
